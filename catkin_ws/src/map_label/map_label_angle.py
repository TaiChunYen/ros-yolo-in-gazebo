...
clustering = DBSCAN(eps=0.4, min_samples=2).fit(np_marker_set)
...
ground_truth = [[-0.279549,-4.74448+0.248],[0.219341,-4.74534+0.248],[-0.775916,-4.75843+0.248]]#tomato_test
#ground_truth = [[-1.4256+0.248,-0.120952],[-1.4256-0.248,-0.120952],[0.453669+0.248,1.91165],[0.453669-0.248,1.91165],[0.453708+0.248,2.42551],[0.453708-0.248,2.42551]]#tomatosim
np_ground_truth = np.array(ground_truth).astype(np.float64)

np_filted_marker = np.array(filted_marker).astype(np.float64)

np_angle_set = np.array(angle_set).astype(np.float64)
...
#print(all_error)
np_all_error = np.array(all_error).astype(np.float64)
combined = np.vstack((np_all_error, np_angle_set)).T
